        ...
        defectStore.load().then({
            success: function(records) {
                ...
                if ( records.length > 0){
                    ...
                    app.add( {
                        xtype: 'text',
                        text: "Total Defects: " + records.length + "\nUnparented Defects: " + app.unparentedDefects,
                        margin: 10
                    });
                }
                else {
                    var contxt = app.getContext();
                    Rally.ui.notify.Notifier.show({message: 'No defects found in ' + contxt.getProject().Name + 'Try a different project or date settings'});
                }
